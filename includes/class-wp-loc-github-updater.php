...
class WP_LOC_GitHub_Updater {

    private const REPOSITORY = 'vitaliikaplia/wp-loc';
    private const CACHE_KEY = 'wp_loc_github_update_data';
    ...
    public function filter_update_plugins_transient( $transient ) {
        ...
        $local_version = $transient->checked[ WP_LOC_BASENAME ] ?? WP_LOC_VERSION;
        ...
        if ( version_compare( $remote_data['version'], $local_version, '>' ) ) {
            $transient->response[ WP_LOC_BASENAME ] = $update;
            unset( $transient->no_update[ WP_LOC_BASENAME ] );
        } else {
            $transient->no_update[ WP_LOC_BASENAME ] = $update;
            unset( $transient->response[ WP_LOC_BASENAME ] );
        }
    ...
    public function filter_plugins_api( $result, string $action, $args ) {
        ...
        $remote_data = $this->get_remote_update_data( $this->should_force_check() );
        $version = $remote_data['version'] ?? WP_LOC_VERSION;
        ...
        return (object) [
            ...
            'sections' => [
                'description' => '<p>WP-LOC is a lightweight multilingual plugin for WordPress with compatible translation links, language-aware routing, localized options, ACF support, Yoast SEO support, and migration tooling.</p>',
                'changelog' => '<p>Updates are loaded from the master branch of the public GitHub repository when the plugin header version is newer than the installed version.</p>',
            ],
        ];
    }
    ...
    private function get_remote_update_data( bool $force = false ): ?array {
        ...
        $response = wp_remote_get(
            ...
            [
                ...
                'headers' => [
                    'Accept' => 'text/plain',
                    'User-Agent' => 'WP-LOC/' . WP_LOC_VERSION . '; ' . home_url( '/' ),
                ],
            ]
        );
    ...
    private function get_slug(): string {
        return dirname( WP_LOC_BASENAME );
    }

    private function get_branch(): string {
        $branch = defined( 'WP_LOC_GITHUB_BRANCH' ) ? (string) WP_LOC_GITHUB_BRANCH : 'master';
        $branch = trim( $branch );
    ...
    private function get_remote_plugin_file_url(): string {
        return sprintf(
            'https://raw.githubusercontent.com/%s/%s/wp-loc.php',
            self::REPOSITORY,
            $this->get_url_branch()
        );
    }
