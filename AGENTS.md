...
- **Block-editor term trees follow the edited post's language**: Gutenberg loads taxonomy collections over REST, where `is_admin()` is false, so the context otherwise resolves to the FRONTEND language and a translated post's editor shows the wrong-language tree — none of the post's own terms then matches a checkbox, and the next save submits an empty set (the wipe contained by the sync guard below). `WP_LOC_Terms::get_rest_edited_post_language()` recovers the post from an explicit `?post=` arg or the editor referer (`post.php?post=ID`). The classic editor renders its metabox on a real admin screen and was never affected; outside an editor REST context nothing changes.
- **Two independent layers must honor the editor language — fixing one leaves the bug fully visible.** (1) `get_context_language()`, which feeds `filter_terms_clauses()` and scopes the SQL; (2) `adjust_term_to_current_language()`, hooked on `get_term` at priority 1, which must return the term untouched in that context. `WP_Term_Query` maps every result row through `get_term()`, so layer 2 silently re-maps each correctly-scoped row back to the default language and the panel still shows the wrong tree even when the query was right (measured: `get_term(71 'Education')` returned `70 'Навчання'`).
- **Known gap — an explicit `lang` term-query arg survives the SQL layer but not the object layer.** `filter_terms_clauses()` honours `$args['lang']`, yet `WP_Term_Query::populate_terms()` re-fetches every selected row through `get_term()`, where `adjust_term_to_current_language()` maps it to the CONTEXT language instead; outside admin and outside an editor REST request, `get_terms(['lang' => 'en'])` therefore returns default-language terms — even for `fields => 'ids'`, because populate_terms runs before `format_terms()`. Nothing in the plugin or the sibling themes passes `lang` to a term query (the `'lang' => $slug` occurrences in `class-wp-loc-admin.php` and `class-wp-loc-terms.php` build language-switcher UI arrays, not query args), so this is latent rather than live. Do NOT paper over it by parking the requested language in a static during the query: the per-term filter has no reliable end-of-query hook to clear it, and that shape of stateful cleverness already produced the memo bug above. A real fix means giving the object layer the query's language explicitly.
- **Never memoize "no editor context" before `REST_REQUEST` exists.** The constant is defined at dispatch, while term queries also run earlier in the same request (`init` and friends). Caching the negative answer from one of those early calls poisons every later call in the request and brings the whole bug back — the resolver must bail out *before* consulting its memo while `REST_REQUEST` is undefined, and cache only inside a real REST dispatch.
...
- **Post term sync**: For translatable taxonomies assigned to translatable posts, term relationships sync across the whole post translation group in both directions. Saving any translation becomes the source of truth; sibling posts receive the mapped term translations in their own language. Synchronization also runs after `set_object_terms` so REST/Gutenberg term updates made after `save_post` are included. Registered terms from another language are removed from the source post, and a missing target translation must never fall back to attaching the source-language term. **An empty source term set is never mirrored onto siblings**: when the saved post has no terms in a taxonomy (or the term lookup fails), the sync skips that taxonomy and leaves every sibling's terms untouched — an editor save whose language-scoped panel failed to see the assigned terms submits an empty set, and replicating it wiped the category relationships of whole translation groups. Deliberately clearing a sibling's terms is done by editing that sibling directly; a source that HAS terms with no mappable translations still clears the target (no source-language fallback).
...
- **ACF none-mode entity fields**: `none` (the default for unconfigured fields) on posts and terms means "not managed by wp-loc": saving the source entity must never push `none` values onto its translations. Translations fall back to the source value on read only while they have no own stored value; once a translation saves its own value, it wins and is never overwritten. Only explicit `shared` mirrors values across the translation group.
...
- **Verify language-context fixes over a real authenticated HTTP request, never with a `wp eval` simulation.** A CLI script that defines `REST_REQUEST` and sets `HTTP_REFERER` up front cannot reproduce request lifecycle order, so it reports success while the browser still fails — that false green shipped two broken releases of the block-editor term fix. Mint credentials and replay the exact request the editor makes:

  ```bash
  # 1) cookie + REST nonce for an administrator (same session token for both)
  wp eval '$uid = 2; $exp = time() + 3600;
    $token = WP_Session_Tokens::get_instance($uid)->create($exp);
    $cookie = wp_generate_auth_cookie($uid, $exp, "logged_in", $token);
    $_COOKIE[LOGGED_IN_COOKIE] = $cookie; wp_set_current_user($uid);
    echo LOGGED_IN_COOKIE . "=" . $cookie . "\n" . wp_create_nonce("wp_rest") . "\n";'

  # 2) the request Gutenberg's term panel actually sends
  curl -s "https://SITE/index.php?rest_route=/wp/v2/categories&per_page=100&_fields=id,name&context=view" \
    -H "Cookie: <cookie-from-step-1>" -H "X-WP-Nonce: <nonce>" \
    -H "Referer: https://SITE/wp-admin/post.php?post=<TRANSLATED_POST_ID>&action=edit"
  ```

  Check all four contexts before calling it fixed: translated-post referer, default-language-post referer, no editor referer (must stay frontend language), and the plain frontend. When a fix appears not to apply, confirm the loaded file first — PHP-FPM OPcache serves a stale copy for up to `opcache.revalidate_freq` seconds (and indefinitely where timestamps are not validated), which looks exactly like a failed fix.
